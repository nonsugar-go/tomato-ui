...
def run_ssh(host: str, user: str, password: str, commands: list, prompt: str,
            enable_cmd: str = None, enable_pass: str = None,
            debug: bool = False):
    p = pexpect.spawn(f"ssh {user}@{host}", encoding="utf-8", timeout=10,
    ...
    if debug:
        print(f"\n[*] Sending SSH password for {user}@{host}")
    p.sendline(password)

    if enable_cmd and enable_pass:
        p.expect(prompt, timeout=60)
        if debug:
            print(f"\n[*] Elevating privileges: {enable_cmd}")
        p.sendline(enable_cmd)
        p.expect(r"[Pp]assword:[ ]?$")
        if debug:
            print(f"\n[*] Sending {enable_cmd} password")
        p.sendline(enable_pass)

    for _ in range(3):
        time.sleep(.2)
        p.sendline("")
    p.expect(f".*{prompt}", timeout=5)

    for cmd in commands:
        p.expect(prompt, timeout=180)
        if debug:
            print(f"\n[*] Executing: {cmd}")
        p.sendline(cmd)
...
def main():
    ...
    parser.add_argument("-prompt", default=r"[$#>][ ]?$",
                        help="プロンプトの正規表現 (デフォルト: '[$#>][ ]?$')")
    parser.add_argument("-enable_cmd",
                        help="特権昇格コマンド (例: enable, sudo -i, expert)")
    parser.add_argument("-enable_pass", help="特権昇格用パスワード")
    ...
    if args.debug:
        ...
        print(f"[*] Enable Command: {args.enable_cmd}")
        print(f"[*] Enable Password: {'*' * len(args.enable_pass)}")
